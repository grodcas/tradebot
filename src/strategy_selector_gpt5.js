/**
 * Strategy Selector - GPT-5 Iter5 (EUR/USD)
 *
 * Coordinates specialist agents using GPT-5.2 model.
...
/**
 * Main entry point - calls multi-agent orchestrator (GPT-5.2)
 */
async function callStrategyTradeDecision({ context, indicators, currentBar, waitCount, mustTrade, waitHistory = [] }) {
  ...
  // Additional context for iter5 orchestrator
  const marketRegime = indicators.marketRegime || null;
  const structureState = indicators.structureState || null;
  const breakoutScore = indicators.breakoutScore || null;
  const sweepScore = indicators.sweepScore || null;

  // Call the multi-agent orchestrator (uses GPT-5.2)
