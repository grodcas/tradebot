...
async function determineLevels({
  ...
  const testThreshold = safeAtr * 0.3;
  let supportTests = 0, resistanceTests = 0;
  for (const p of priceClose) {
    if (Math.abs(p - safeSupport) < testThreshold) supportTests++;
    if (Math.abs(p - safeResistance) < testThreshold) resistanceTests++;
  }
...
KEY LEVELS:
- Support: ${safeSupport.toFixed(5)} (tested ~${supportTests} times in recent bars)
- Resistance: ${safeResistance.toFixed(5)} (tested ~${resistanceTests} times in recent bars)
...
VOLATILITY:
- ATR (5min): ${safeAtr.toFixed(5)}
- ATR as % of price: ${(safeAtr / currentPrice * 100).toFixed(3)}%
